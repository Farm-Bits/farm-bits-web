class PlcVersionSerializer < Blueprinter::Base
  ...
  field :group_labels do |plc_version|
    group_names =
      if plc_version.register_templates.loaded?
        plc_version.register_templates.map(&:group_name).compact_blank.uniq
      else
        plc_version.register_templates.where.not(group_name: [nil, '']).distinct.pluck(:group_name)
      end
